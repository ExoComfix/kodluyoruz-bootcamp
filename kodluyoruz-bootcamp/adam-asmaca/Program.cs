...
namespace adam_asmaca
{
    internal class Program
    {
        ...
        private static int Play(int life,int level)
        {
            ...
            Console.WriteLine($"Seviye : {level+1}");
            while (!isWordFinding)
            {
                DrawGame(life);
        ...
        static string ChooseWord(int level)
        {
            ...
            {
                case 0:    
                    string[] words = { "lavanta", "banka", "fare", "kedi" };
                    word = words[random.Next(0, words.Length)];
                    return word;
                case 1:
                    random = new Random();
                    string[] words1 = { "yemek", "kolluk", "telefon", "bardak" };
                    word = words1[random.Next(0, words1.Length)];
                    return word;
                case 2:
                    random = new Random();
                    string[] words2 = { "madrid", "maldiv", "malezya", "motor" };
                    word = words2[random.Next(0, words2.Length)];
                    return word;
                case 3:
                    random = new Random();
                    string[] words3 = { "halı", "yatak", "dolap", "psikoanaliz" };
